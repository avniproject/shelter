...
-- MIGRATION 6 Update the Household subject_type type from Group to Household
with audits as (
    update subject_type set type = 'Household', is_group = true, is_household = true where name = 'Household'
        returning audit_id
)
update audit
set last_modified_date_time = current_timestamp(3)
where id = (select audit_id from audits);

--Migration 7 Change role Family member to "Member" and add another "Head of household" role
with audits as (update group_role
    set role = 'Member'
    where group_subject_type_id = (select id from subject_type where name = 'Household')
        and role = 'Family Member'
    returning audit_id)
update audit
set last_modified_date_time = current_timestamp(3)
where id = (select audit_id from audits);

insert into group_role (uuid, group_subject_type_id, role, member_subject_type_id, maximum_number_of_members,
                        minimum_number_of_members, organisation_id, audit_id, version)
values (uuid_generate_v4(),
        (select id from subject_type where name = 'Household'),
        'Head of household',
        (select id from subject_type where name = 'Family Member'),
        1,
        1,
        (select id from organisation where name = 'Shelter'),
        create_audit((select id from users where username = 'shubhangi@shelter')),
...
--Migration 8 make the first member head of the household
with first_group_subjects as (
    select first_value(id) over (partition by group_subject_id order by id) as first_id
    from group_subject
    where not is_voided
),
     audits as (
         update group_subject set group_role_id = (select id from group_role where role = 'Head of household')
             where id in (select first_id from first_group_subjects)
             returning audit_id
     )
update audit
set last_modified_date_time = current_timestamp(3) + (id / 1000) * interval '1 millisecond'
...
-- Add relationships from the app designer.

-- Relationships are not migrated and user can add it later by editing the member.
